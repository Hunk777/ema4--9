...
input group "===== その他 ====="
input int InpMagic = 10002;                       // マジックナンバー
...
void OpenPosition(int signal)
{
    ...
    if(signal == 1)
    {
        result = trade.Buy(InpLots, _Symbol, 0, 0, 0, "e2_BUY");
    }
    else if(signal == -1)
    {
        result = trade.Sell(InpLots, _Symbol, 0, 0, 0, "e2_SELL");
    }
...
void CreatePanel()
{
    ObjectCreate(0, "EMA_Panel_BG", OBJ_RECTANGLE_LABEL, 0, 0, 0);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_CORNER, CORNER_LEFT_UPPER);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_XDISTANCE, 10);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_YDISTANCE, 30);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_XSIZE, 250);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_YSIZE, 120);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_BGCOLOR, C'20,20,30');
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_BORDER_TYPE, BORDER_FLAT);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_COLOR, clrDodgerBlue);
    ObjectSetInteger(0, "EMA_Panel_BG", OBJPROP_WIDTH, 2);
}
...
void UpdatePanel()
{
    ...
    {
        string objName = "EMA_Panel_L" + IntegerToString(i);
...
{
    // 既存ライン削除
    ObjectsDeleteAll(0, "EMA_Fast_");
    ObjectsDeleteAll(0, "EMA_Slow_");
